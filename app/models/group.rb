  ...
  def update_memberships
    if parents_of
      ...
      link_code.downcase.split.each do |code|
        conditions.add_condition ["lower(classes) = ? or classes #{sql_ilike} ? or classes #{sql_ilike} ? or classes #{sql_ilike} ? or classes #{sql_ilike} ? or classes #{sql_ilike} ?", code, "#{code},%", "%,#{code}", "%,#{code},%", "#{code}[%", "%,#{code}[%"], 'or'
      end
