  ...
  class << self
    ...
    def get_hash_of_settings_in_db(site_id=nil)
      if DB_ADAPTER.mysql?
        ...
        Setting.connection.select_all("select md5(lower(array_to_string(array(select section || name from settings where #{site_id ? ('site_id = ' + site_id.to_s) : 'global = true'}), ','))) as hash").first['hash']
      end
    end
